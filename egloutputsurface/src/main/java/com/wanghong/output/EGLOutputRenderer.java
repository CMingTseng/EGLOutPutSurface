...
public class EGLOutputRenderer implements GLSurfaceView.Renderer {
    ...
    private int mOutputRenderType = OUTPUT_RENDER_TYPE_STILL_BITMAP;
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        mTexture = createTexture(GLES11Ext.GL_TEXTURE_EXTERNAL_OES);
        ...
        if (eglOutputSurfaceCallback != null) {
            eglOutputSurfaceCallback.onOutputSurfaceCreated(mSurfaceTexture);
        }
    }
...
return shader;
    ...
    private int createTexture(int target) {
        ...
        if (target == GLES20.GL_TEXTURE_2D && sBitmap != null && !sBitmap.isRecycled()) {
            GLUtils.texImage2D(target, 0, sBitmap, 0);
        }
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (mOutputRenderType == OUTPUT_RENDER_TYPE_CONTINUOUS_PICTURES) {
        ...
        int positionIndex = GLES20.glGetAttribLocation(mProgram, "aPosition");
        int texcoordIndex = GLES20.glGetAttribLocation(mProgram, "aTexCoord");
        ...
        int tex = 0;
        if (mOutputRenderType == OUTPUT_RENDER_TYPE_STILL_BITMAP) {
            tex = 1;
        } else if (mOutputRenderType == OUTPUT_RENDER_TYPE_CONTINUOUS_PICTURES) {
            tex = 0;
        }
        GLES20.glUniform1i(GLES20.glGetUniformLocation(mProgram, "tex"), tex);
        GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
        ...
        int sTexture2D = GLES20.glGetUniformLocation(mProgram, "sTexture2D");
    ...
    public void setStillBitmap(Bitmap stillBitmap) {
        this.sBitmap = stillBitmap;
        requestRenderStillBitmap();
    }

    private void requestRenderStillBitmap() {
        mTexture2D = createTexture(GLES20.GL_TEXTURE_2D);
        if (attachedGLSurfaceView != null && attachedGLSurfaceView.get() != null) {
            attachedGLSurfaceView.get().requestRender();
        }
    }
    ...
    public void setOutputRenderType(int outputRenderType) {
        this.mOutputRenderType = outputRenderType;
    }
}
